...
#         echo 'Unzip installing complete...'
#     fi
#     echo 'Unzip is updated.'
#     curl -fsSL https://deno.land/install.sh | sh
# else echo "Deno installed before."
# fi 

# # Install brili.ts through deno
# if ! command -v brili >/dev/null 2>&1; then
#   echo "Installing brili..."
#   cd bril/ || { echo 'Failed to cd'; exit 1;}
#   export PATH="$HOME/.deno/bin:$PATH"
#   echo -e 'export PATH="$HOME/.deno/bin:$PATH"' >> ~/.bashrc
#   deno install -g ./brili.ts
# else echo "brili installed before."
# fi
# # Install flit

# if ! command -v flit >/dev/null 2>&1; then
#   echo "Installing flit..."
#   export PATH="$HOME/.local/bin:$PATH"
#   echo -e 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.bashrc
#   pip install --user flit
# else echo "flit installed before."
# fi

# # Install symlinks to the tools (including bril2json, bril2txt)

# if ! command -v bril2json >/dev/null 2>&1; then
#   echo "Installing symlinks to bril tools..."
#   cd bril-txt/
#   flit install --symlink --user
# else echo "bril2json installed before."
# fi